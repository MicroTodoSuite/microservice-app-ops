...
- [ ] T012 Restructure the `fdev`, `fstg`, `fprd`, and egress roots in code to the same layout — partial: `shd/networking` replaces the shared egress root with the single-NAT hub through `network-v1.0.0` and `transit-egress-v1.0.0`; the nine `fdev`, `fstg`, and `fprd` roots remain
