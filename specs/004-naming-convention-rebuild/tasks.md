...
- [ ] T011 Write the `eco` roots in the PC-IAC-022 domain order, consuming the account through `var.aws_account_id` — partial: `eco/networking` is written, consuming `network-v1.0.0` and reading shd/security's flow-log key and role by name, and the reusable IaC gate runs on `aws/environments/eco`; `eco/security` is written with the cluster and node roles, the secrets and control-plane log keys, the cluster and node security groups, the six Secrets Manager containers, and the EKS Pod Identity roles of the `vpc-cni` and `aws-ebs-csi-driver` add-ons, so the CNI is authorized before the cluster's OIDC issuer exists; `eco/workload` is written with `lex-mts-eco-eks-main` through `eks-cluster-v1.1.0`, its managed add-ons with the Pod Identity agent, and the bootstrap node group through `eks-node-group-v1.0.0`; the IRSA pass for the in-cluster applications follows
