...
- [ ] T012 Restructure the `fdev`, `fstg`, `fprd`, and egress roots in code to the same layout — partial: `shd/networking` replaces the shared egress root with the single-NAT hub through `network-v1.0.0` and `transit-egress-v1.0.0`, and `fdev/networking` now provides the three-zone transit-only spoke through `network-v1.0.0`; the remaining eight full-profile roots remain
