...
- [ ] T012 Restructure the `fdev`, `fstg`, `fprd`, and egress roots in code to the same layout — partial: `shd/networking` replaces the shared egress root with the single-NAT hub through `network-v1.0.0` and `transit-egress-v1.0.0`, `fdev/networking`, `fstg/networking`, and `fprd/networking` provide their three-zone transit-only spokes, `fdev/security` plus `fdev/workload` provide the full-development security and workload roots, `fstg/security` plus `fstg/workload` provide the full-staging security and workload roots, and `fprd/security` now provides the full-production security root; the remaining fprd workload root remains
