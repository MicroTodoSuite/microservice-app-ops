...
- [ ] T011 Write the `eco` roots in the PC-IAC-022 domain order, consuming the account through `var.aws_account_id` — partial: `eco/networking` is written, consuming `network-v1.0.0` and reading shd/security's flow-log key and role by name, and the reusable IaC gate runs on `aws/environments/eco`; `eco/security`, `eco/workload`, and the IRSA pass follow
