...
- [ ] T010 Write the `shd` roots, with the GitHub OIDC provider and the public zone imported rather than created — partial: `shd/state` is written, consuming `state-backend-v1.0.0`, and the reusable IaC gate runs on `aws/environments/shd`; `shd/security` is written with the GitHub OIDC provider imported, the image publisher role, and the flow-log key and role, while the Terraform deploy role and the Kyverno verifier are still to come; `shd/registry` and `shd/dns` follow
