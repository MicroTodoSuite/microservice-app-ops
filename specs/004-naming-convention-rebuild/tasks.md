...
- [ ] T009 Decompose `environment-foundation` into the modules of the target layout, each with the PC-IAC-001 layout, `sample/`, and `tests/` — partial: twelve modules in `terraform-aws-modules`, each with the PC-IAC-001 files, `sample/`, `tests/`, and a release tag (`ecr-repository`, `eks-cluster` v1.1.0, `eks-node-group`, `iam-oidc-provider`, `iam-role`, `kms-key`, `network`, `route53-zone`, `secret`, `security-group`, `state-backend`, `transit-egress`), cover every resource the rebuilt roots create; the legacy module's opt-in Karpenter prerequisites (interruption queue and its key, EventBridge rules, controller and node identities) have no module yet, and the full profile needs them (gitops spec 009 T086)
