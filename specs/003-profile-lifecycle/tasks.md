...
- [ ] **T016 Bring a profile down without a GitOps pull request** — in progress; scoped in microservice-app-ops#120
  ...
  - [X] Commit a failing contract in `tests/contract/aws-profile-lifecycle.sh`: a down plan is refused without a quiescence receipt, and refused when the receipt predates the volume record — red commit `test(lifecycle): specify post-destroy runtime sweep and quiescence receipt contracts`, extended with missing/wrong-cluster/malformed tag negatives, tamper bindings, ordered event log, snapshot gate, and idempotent partial-failure rerun
  ...
  - [X] Implement the chosen mechanism so that only runtime-class resources are reachable, reusing the durable and egress delete filters — `execute_post_destroy_sweep` runs between workload destruction and the first networking apply with an explicit type allow-list, exact live tag revalidation, and not-found-only tolerance; durable filter extended to the ACM, Route53, ECR, Secrets, KMS, S3, and GitHub OIDC companion families
  ...
  - [X] Document the operator flow and the residual durable cost in `docs/aws-profile-lifecycle.md` — shutdown section rewritten for snapshot, receipt, plan, and sweep; residual durable cost paragraph unchanged
