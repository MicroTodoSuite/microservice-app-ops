...
azure/environments/eco/workload/       # RG, storage account, two containers (T014)
  tests/workload.tftest.hcl            # failing contract (T011)
azure/environments/eco/security/       # replication identity, role assignments (T015)
  tests/security.tftest.hcl            # failing contract (T011)
tests/contract/azure-eco-backups.sh    # source contract (T012)
tests/workflows/eco-state-replication.bats  # workflow contract (T013)
.github/workflows/eco-state-replication.yml # replication job (T017)
docs/eco-off-provider-recovery.md      # drill runbook (T027)
```
...
- **Before implementation**: the four contracts fail, which is the evidence of
  this pull request.
- **Live**: the first dispatched replication run (T020), and the drill (T030).
- **AWS plan invariance**: nothing here changes an `aws/` root until T016 and
  T023-T024; T019 and the implementing pull requests confirm from saved-plan
  JSON that only the approved identity and empty secret resources are added.

## Documentation consulted (re-verified 2026-09-22)
...
- Vendor site, because no documentation MCP server covers the plugin's current
  compatibility table: `velero-io/velero-plugin-for-microsoft-azure` README (authentication
