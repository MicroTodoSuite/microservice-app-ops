...
project has already lost two AWS accounts (ADR-0001). If a third loss happened
...
- **Q**: How is the Azure storage root structured under the IaC rules?
  **A**: As live roots that consume modules from `terraform-azure-modules` by
  ...
  no module on its `main` on 2026-09-22 (only `_template/`; its pull request #2
...
- **SC-005**: One dispatched run writes a manifest whose every SHA-256 matches
  the S3 source, and the workflow run URL is recorded in T020.
