...
run "kyverno_verifier_contract" {
  ...
  assert {
    condition     = toset(jsondecode(aws_iam_role_policy.kyverno_ecr_verifier.policy).Statement[1].Resource) == toset([for service in var.neutral_service_names : "arn:aws:ecr:us-east-1:995253610162:repository/microtodosuite/${service}"])
