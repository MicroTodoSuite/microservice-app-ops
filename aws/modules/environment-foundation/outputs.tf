output "foundation_contract" {
  ...
  value = {
    ...
    network = {
      ...
      private_worker_subnet_count = length(try(module.vpc[0].private_subnets, []))
      public_subnet_count         = length(try(module.vpc[0].public_subnets, []))
      nat_gateway_count           = length(try(module.vpc[0].natgw_ids, []))
      ...
      internet_gateway_serves_public_load_balancers = var.runtime_enabled
    ...
    cluster_prerequisites = {
      enabled = var.runtime_enabled && var.enable_full_profile_cluster_prerequisites
      ...
      ebs_csi_driver = var.runtime_enabled
      ...
      vpc_cni_network_policy_enabled = var.runtime_enabled
    }
