...
variable "enable_full_profile_tooling_secrets" {
  description = "Whether the owning foundation creates the three full-profile secret containers: the Grafana administrator and the SonarQube database and administrator credentials. Off by default so no environment gains a secret before the stage that consumes it."
  type        = bool
  default     = false
}

variable "full_profile_secret_values" {
  description = "Ephemeral values for the full-profile secret containers, keyed by grafana_admin, sonarqube_db and sonarqube_admin. Generated by the composition root from `ephemeral \"random_password\"` and consumed only by write-only arguments, so no value reaches the plan or the state."
  type        = map(string)
  sensitive   = true
  ephemeral   = true
  default     = {}
  ...
  validation {
    condition = !var.enable_full_profile_tooling_secrets || !var.create_shared_resources || toset(keys(var.full_profile_secret_values)) == toset([
      "grafana_admin",
      "sonarqube_admin",
      "sonarqube_db",
    ])
    error_message = "full_profile_secret_values must contain exactly grafana_admin, sonarqube_db and sonarqube_admin when the owning foundation enables the full-profile tooling secrets."
  }
}

variable "full_profile_secret_versions" {
  description = "Non-secret rotation counters for the full-profile secret containers, keyed exactly like full_profile_secret_values. Terraform re-evaluates the ephemeral value on every apply, so incrementing a counter here is the only thing that rotates a stored secret."
  type        = map(number)
  default     = {}

  validation {
    condition = alltrue([
      for key, version in var.full_profile_secret_versions :
      version >= 1 && floor(version) == version
    ])
    error_message = "Every full_profile_secret_versions entry must be a positive integer."
  }

  validation {
    condition     = !var.enable_full_profile_tooling_secrets || !var.create_shared_resources || toset(keys(var.full_profile_secret_versions)) == toset(keys(var.full_profile_secret_values))
    error_message = "full_profile_secret_versions must carry one counter for exactly the same keys as full_profile_secret_values."
  }
}

variable "sonarqube_service_account_subject" {
  description = "Exact EKS service-account subject allowed to read the SonarQube database and administrator secrets."
  type        = string
  default     = "system:serviceaccount:sonarqube:sonarqube-external-secrets-jwt"

  validation {
    condition     = var.sonarqube_service_account_subject == "system:serviceaccount:sonarqube:sonarqube-external-secrets-jwt"
    error_message = "sonarqube_service_account_subject must identify only the sonarqube namespace's External Secrets ServiceAccount."
  }
}

variable "sonarqube_reader_issuer_label" {
  description = "Key of additional_eks_oidc_issuers naming the ONE cluster that hosts the shared SonarQube instance. SonarQube is a single full-dev tenant, so its reader trusts exactly that issuer rather than the shared multi-issuer set. Null disables the reader."
  type        = string
  default     = null

  validation {
    condition     = var.sonarqube_reader_issuer_label == null || contains(keys(var.additional_eks_oidc_issuers), coalesce(var.sonarqube_reader_issuer_label, "-"))
    error_message = "sonarqube_reader_issuer_label must name an entry of additional_eks_oidc_issuers, so the reader cannot be pointed at an issuer this foundation has not reviewed."
  }
}

variable "consumer_jwt_environment" {
  description = "For a consumer foundation only: the ONE environment whose owner-held JWT secret this cluster may read. It creates a cluster-qualified reader role and no secret at all. Null means this cluster reads no JWT secret."
  type        = string
  default     = null

  validation {
    condition     = var.consumer_jwt_environment == null || can(regex("^[a-z0-9-]+$", var.consumer_jwt_environment))
    error_message = "consumer_jwt_environment must be null or a DNS-safe environment name."
  }

  validation {
    condition     = var.consumer_jwt_environment == null || !var.create_shared_resources
    error_message = "consumer_jwt_environment belongs to a consumer foundation; the owning foundation already creates a reader for every shared environment."
  }
}

variable "enable_dr_secret_seed" {
  description = "Whether the opt-in Azure DR seed role exists. It is the only identity in this account permitted to read secret VALUES, so it stays off until disaster recovery is approved."
  type        = bool
  default     = false

  validation {
    condition     = !var.enable_dr_secret_seed || !var.create_shared_resources || var.enable_full_profile_tooling_secrets
    error_message = "The DR seed reads the Grafana administrator secret, so enable_full_profile_tooling_secrets must be on before the seed role can exist."
  }
}

variable "dr_secret_seed_subjects" {
  description = "Exact GitHub Actions subjects allowed to assume the DR seed role, of the form repo:<owner>/<repo>:environment:<environment>. Pinning the environment means the seed can only run from a job that passed that environment's approval gate."
  type        = set(string)
  default     = []

  validation {
    condition = alltrue([
      for subject in var.dr_secret_seed_subjects :
      can(regex("^repo:[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+:environment:[A-Za-z0-9_.-]+$", subject))
    ])
    error_message = "Each dr_secret_seed_subjects entry must pin an exact repository AND environment."
  }

  validation {
    condition     = !var.enable_dr_secret_seed || !var.create_shared_resources || length(var.dr_secret_seed_subjects) > 0
    error_message = "The DR seed role must not exist with an unbounded trust; name the exact repository and environment allowed to assume it."
  }
}

variable "dr_secret_seed_workflow_refs" {
  description = "Exact GitHub Actions job_workflow_ref values allowed to assume the DR seed role. Combined with dr_secret_seed_subjects this pins repository, environment and workflow file, so no other job in the same repository can read these values."
  type        = set(string)
  default     = []

  validation {
    condition = alltrue([
      for ref in var.dr_secret_seed_workflow_refs :
      can(regex("^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+/.+\\.ya?ml@refs/(heads|tags)/.+$", ref))
    ])
    error_message = "Each dr_secret_seed_workflow_refs entry must be a fully qualified <owner>/<repo>/<path>.yml@refs/heads/<branch> workflow reference."
  }

  validation {
    condition     = !var.enable_dr_secret_seed || !var.create_shared_resources || length(var.dr_secret_seed_workflow_refs) > 0
    error_message = "The DR seed role must name the exact workflow file allowed to assume it."
  }
}
