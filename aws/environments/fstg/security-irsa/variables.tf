# Inputs of the fstg/security-irsa root. Values come from the gitignored fstg.tfvars; the account
...
variable "environment" {
  type        = string
  description = "Environment code, from MTS-IAC-101. This root is the full staging environment's IRSA pass."

  validation {
    condition     = var.environment == "fstg"
    error_message = "This root builds the full staging environment, fstg."
  }
}
...
variable "aws_region" {
  type        = string
  description = "The AWS region of the full staging environment."
...
variable "jwt_reader_namespaces" {
  type        = map(string)
  description = "Kubernetes namespace of each application environment, keyed by its three-letter code, such as dev = microtodo-dev; its external-secrets-jwt service account reads <client>-<project>-fstg-sm-jwt<code>."
