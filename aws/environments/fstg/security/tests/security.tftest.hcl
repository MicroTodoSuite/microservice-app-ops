# Plan-time tests of the fstg/security root against a mocked AWS provider (PC-IAC-018).
mock_provider "aws" {
  alias           = "principal"
...
run "builds_the_fstg_security_names" {
  command = plan

  assert {
    condition     = local.cluster_role_name == "lex-mts-fstg-role-cluster" && local.node_role_name == "lex-mts-fstg-role-node" && local.secrets_key_name == "lex-mts-fstg-kms-eks" && local.logs_key_name == "lex-mts-fstg-kms-ekslogs"
    error_message = "The root must build the fstg standard names (MTS-IAC-101)."
  }

  assert {
    condition     = local.jwt_secret_names == { dev = "lex-mts-fstg-sm-jwtdev", stg = "lex-mts-fstg-sm-jwtstg", prd = "lex-mts-fstg-sm-jwtprd", dmo = "lex-mts-fstg-sm-jwtdmo" } && local.webhook_secrets["slacksec"].name == "lex-mts-fstg-sm-slacksec"
...
run "trusts_only_each_addon_service_account_through_pod_identity" {
  ...
  assert {
    condition     = local.addon_pod_identities["vpccni"].role_name == "lex-mts-fstg-role-vpccni" && local.addon_pod_identities["ebscsi"].role_name == "lex-mts-fstg-role-ebscsi"
  ...
  assert {
    condition     = jsondecode(local.addon_trust_policies["vpccni"]).Statement[0].Condition.StringEquals == { "aws:RequestTag/eks-cluster-arn" = "arn:aws:eks:us-east-1:123456789012:cluster/lex-mts-fstg-eks-main", "aws:RequestTag/kubernetes-namespace" = "kube-system", "aws:RequestTag/kubernetes-service-account" = "aws-node" }
...
run "limits_the_logs_key_to_the_control_plane_log_group" {
  ...
  assert {
    condition     = jsondecode(local.logs_key_policy).Statement[1].Principal.Service == "logs.us-east-1.amazonaws.com" && jsondecode(local.logs_key_policy).Statement[1].Condition.ArnEquals["kms:EncryptionContext:aws:logs:arn"] == "arn:aws:logs:us-east-1:123456789012:log-group:/aws/eks/lex-mts-fstg-eks-main/cluster"
