# Names, access, add-ons, and tags of the fstg/workload root, built here and nowhere else
...
locals {
  ...
  # The names fstg/networking and fstg/security gave what this root reads.
  ...
  # The AWS add-ons that call AWS take fstg/security's Pod Identity roles, so the CNI is
