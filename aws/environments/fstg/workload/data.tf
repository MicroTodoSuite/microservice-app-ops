# What the fstg/workload root reads rather than creates (PC-IAC-017): the network of
# fstg/networking and the roles, keys, and security groups of fstg/security, each found by its
# standard name.
...
data "aws_subnets" "private" {
  ...
  lifecycle {
    postcondition {
      ...
      error_message = "The cluster needs private subnets in at least two Availability Zones, and fstg/networking tags one per zone."
    }
  }
}
