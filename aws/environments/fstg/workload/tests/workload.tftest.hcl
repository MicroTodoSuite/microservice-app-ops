# Plan-time tests of the fstg/workload root against a mocked AWS provider (PC-IAC-018).
mock_provider "aws" {
  alias           = "principal"
  ...
  mock_data "aws_iam_role" {
    defaults = { arn = "arn:aws:iam::123456789012:role/lex-mts-fstg-role-cluster" }
  }
  ...
  mock_resource "aws_eks_cluster" {
    defaults = {
      arn                   = "arn:aws:eks:us-east-1:123456789012:cluster/lex-mts-fstg-eks-main"
...
override_module {
  ...
  outputs = { node_group_arn = "arn:aws:eks:us-east-1:123456789012:nodegroup/lex-mts-fstg-eks-main/lex-mts-fstg-ng-bootstrap/00000000-0000-0000-0000-000000000000" }
}

override_data {
  target = data.aws_iam_role.addon["vpccni"]
  values = { arn = "arn:aws:iam::123456789012:role/lex-mts-fstg-role-vpccni" }
}

override_data {
  target = data.aws_iam_role.addon["ebscsi"]
  values = { arn = "arn:aws:iam::123456789012:role/lex-mts-fstg-role-ebscsi" }
}
...
run "builds_the_fstg_workload_names" {
  command = plan

  assert {
    condition     = local.cluster_name == "lex-mts-fstg-eks-main" && local.node_group_name == "lex-mts-fstg-ng-bootstrap" && local.launch_template_name == "lex-mts-fstg-lt-bootstrap" && local.control_plane_log_group.standard_name == "lex-mts-fstg-cwl-eks"
    error_message = "The root must build the fstg standard names (MTS-IAC-101)."
  }

  assert {
    condition     = local.cluster_security_group_name == "lex-mts-fstg-sg-cluster" && local.node_security_group_name == "lex-mts-fstg-sg-node" && local.addon_role_names == { vpccni = "lex-mts-fstg-role-vpccni", ebscsi = "lex-mts-fstg-role-ebscsi" }
    error_message = "The root must read fstg/security's groups and roles by the names that root gave them."
  }
}
...
run "gives_the_aws_addons_their_pod_identity_roles" {
  ...
  assert {
    condition     = local.addons["vpc-cni"].pod_identity_associations == { aws-node = "arn:aws:iam::123456789012:role/lex-mts-fstg-role-vpccni" } && local.addons["aws-ebs-csi-driver"].pod_identity_associations == { ebs-csi-controller-sa = "arn:aws:iam::123456789012:role/lex-mts-fstg-role-ebscsi" }
