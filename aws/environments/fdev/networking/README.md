...
The full-development VPC is a transit-egress spoke of the shared `shd`
...
and fdev's dedicated route table by their standard `Name` tags. It also reads
...
The reviewed fdev allocation is `10.40.0.0/16`, with public `/24` and private
...
```bash
cp fdev.tfvars.example fdev.tfvars
cp networking.s3.tfbackend.example networking.s3.tfbackend
terraform -chdir=aws/environments/fdev/networking init -backend-config=networking.s3.tfbackend
terraform -chdir=aws/environments/fdev/networking plan -input=false -var-file=fdev.tfvars -out=fdev-networking.tfplan
```
...
security and workload roots for fdev.
