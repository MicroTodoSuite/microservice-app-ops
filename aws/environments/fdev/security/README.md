...
The full-development environment's security root, second in the PC-IAC-022 order
(ops spec 004 T011, ai-agents specs/001 T025). It holds what `fdev/workload`
needs before it can create the cluster.

| Resource | Module | Name |
| --- | --- | --- |
| Control-plane role: `AmazonEKSClusterPolicy`, and use of the secrets key | `iam-role-v1.0.0` | `lex-mts-fdev-role-cluster` |
| Managed nodes' role: `AmazonEKSWorkerNodePolicy`, `AmazonEC2ContainerRegistryPullOnly` | `iam-role-v1.0.0` | `lex-mts-fdev-role-node` |
| Pod Identity role of the `vpc-cni` add-on: `AmazonEKS_CNI_Policy` | `iam-role-v1.0.0` | `lex-mts-fdev-role-vpccni` |
| Pod Identity role of the `aws-ebs-csi-driver` add-on: `AmazonEBSCSIDriverPolicy` | `iam-role-v1.0.0` | `lex-mts-fdev-role-ebscsi` |
| Key for Kubernetes secrets | `kms-key-v1.0.0` | `alias/lex-mts-fdev-kms-eks` |
| Key for the control-plane log group | `kms-key-v1.0.0` | `alias/lex-mts-fdev-kms-ekslogs` |
| Extra control-plane security group | `security-group-v1.0.0` | `lex-mts-fdev-sg-cluster` |
...
- The logs key lets CloudWatch Logs in this region use it only for
  `/aws/eks/lex-mts-fdev-eks-main/cluster`.
...
`lex-mts-fdev-eks-main`, through the session tags Pod Identity sets
(`eks-cluster-arn`, `kubernetes-namespace`, `kubernetes-service-account`).
`fdev/workload` associates the roles through `eks-cluster`'s
`pod_identity_associations`, with the `eks-pod-identity-agent` add-on.

**Not here, because they come in the IRSA pass after `fdev/workload`** (it
...
terraform -chdir=aws/environments/fdev/security init -backend-config=security.s3.tfbackend
terraform -chdir=aws/environments/fdev/security plan -input=false -var-file=fdev.tfvars -out=fdev-security.tfplan
```

An apply uses only that saved plan, after the maintainer approves it
(MTS-IAC-107), and after `fdev/networking`. The state key is new, so the apply
