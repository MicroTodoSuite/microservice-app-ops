# Inputs of the fdev/security root. Values come from the gitignored fdev.tfvars; the account is
...
variable "environment" {
  type        = string
  description = "Environment code, from MTS-IAC-101. This root is the full-development environment's security."

  validation {
    condition     = var.environment == "fdev"
    error_message = "This root builds the full-development environment, fdev."
  }
}
...
variable "aws_region" {
  type        = string
  description = "The AWS region of the full-development environment."
...
variable "jwt_environment_codes" {
  type        = list(string)
  description = "Three-letter codes of the application environments the shared cluster hosts, such as dev, stg, prd, and dmo; each gets its JWT signing secret, <client>-<project>-fdev-sm-jwt<code>."

  validation {
    condition     = length(var.jwt_environment_codes) > 0 && length(distinct(var.jwt_environment_codes)) == length(var.jwt_environment_codes) && alltrue([for code in var.jwt_environment_codes : can(regex("^[a-z]{3}$", code))])
    error_message = "Give distinct codes of three lowercase letters."
  }
}
