# Plan-time tests of the fdev/security-irsa root against a mocked AWS provider (PC-IAC-018).
mock_provider "aws" {
  alias           = "principal"
  ...
  mock_data "aws_secretsmanager_secret" {
    defaults = { arn = "arn:aws:secretsmanager:us-east-1:123456789012:secret:lex-mts-fdev-sm-other-AbCdEf" }
  }
  ...
  mock_data "aws_iam_role" {
    defaults = { arn = "arn:aws:iam::123456789012:role/lex-mts-fdev-role-node" }
  }
...
override_data {
  ...
  values = { arn = "arn:aws:secretsmanager:us-east-1:123456789012:secret:lex-mts-fdev-sm-jwtdev-AbCdEf" }
}
...
run "grants_each_role_only_what_its_application_reads" {
  ...
  assert {
    condition     = jsondecode(local.irsa_roles["jwtdev"].policy).Statement[0].Resource == "arn:aws:secretsmanager:us-east-1:123456789012:secret:lex-mts-fdev-sm-jwtdev-AbCdEf" && toset(jsondecode(local.irsa_roles["jwtdev"].policy).Statement[0].Action) == toset(["secretsmanager:DescribeSecret", "secretsmanager:GetSecretValue"])
    error_message = "A JWT reader may only describe and read its own environment's secret."
  }

  assert {
    condition     = jsondecode(local.irsa_roles["secsecret"].policy).Statement[0].Resource == "arn:aws:secretsmanager:us-east-1:123456789012:secret:lex-mts-fdev-sm-slacksec-AbCdEf"
...
run "gives_the_karpenter_controller_its_reference_permissions" {
  ...
  assert {
    condition     = one([for statement in jsondecode(local.irsa_roles["karpenter"].policy).Statement : statement if statement.Sid == "AllowInterruptionQueueActions"]).Resource == "arn:aws:sqs:us-east-1:123456789012:lex-mts-fdev-sqs-karpenter" && one([for statement in jsondecode(local.irsa_roles["karpenter"].policy).Statement : statement if statement.Sid == "AllowPassingInstanceRole"]).Resource == "arn:aws:iam::123456789012:role/lex-mts-fdev-role-node"
  ...
  assert {
    condition     = one([for statement in jsondecode(local.irsa_roles["karpenter"].policy).Statement : statement if statement.Sid == "AllowScopedDeletion"]).Condition.StringEquals == { "aws:ResourceTag/kubernetes.io/cluster/lex-mts-fdev-eks-main" = "owned" } && one([for statement in jsondecode(local.irsa_roles["karpenter"].policy).Statement : statement if statement.Sid == "AllowAPIServerEndpointDiscovery"]).Resource == "arn:aws:eks:us-east-1:123456789012:cluster/lex-mts-fdev-eks-main"
