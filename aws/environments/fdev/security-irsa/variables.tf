# Inputs of the fdev/security-irsa root. Values come from the gitignored fdev.tfvars; the account
...
variable "environment" {
  type        = string
  description = "Environment code, from MTS-IAC-101. This root is the full development environment's IRSA pass."
...
variable "jwt_reader_namespaces" {
  type        = map(string)
  description = "Kubernetes namespace of each application environment, keyed by its three-letter code, such as dev = microtodo-dev; its external-secrets-jwt service account reads <client>-<project>-fdev-sm-jwt<code>."
