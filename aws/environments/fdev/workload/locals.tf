# Names, access, add-ons, and tags of the fdev/workload root, built here and nowhere else
...
locals {
  ...
  # The names fdev/networking and fdev/security gave what this root reads.
  ...
  # The AWS add-ons that call AWS take fdev/security's Pod Identity roles, so the CNI is
