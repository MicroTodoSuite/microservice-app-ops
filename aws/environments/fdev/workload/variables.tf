# Inputs of the fdev/workload root. Values come from the gitignored fdev.tfvars; the account is
...
variable "environment" {
  type        = string
  description = "Environment code, from MTS-IAC-101. This root is the full-development environment's workload."
...
variable "endpoint_public_access_cidrs" {
  type        = list(string)
  description = "Operator CIDR blocks allowed to reach the public API endpoint, such as /32 addresses; [] keeps the API private. Real addresses belong only in the gitignored fdev.tfvars."
