...
The full-development environment's workload, third in the PC-IAC-022 order (ops
spec 004 T011, ai-agents specs/001 T025). It creates the cluster and its
bootstrap capacity.

| Resource | Module | Name |
| --- | --- | --- |
| EKS cluster, Kubernetes `kubernetes_version`, private API plus an optional public endpoint | `eks-cluster-v1.1.0` | `lex-mts-fdev-eks-main` |
| Control-plane log group, encrypted with fdev/security's logs key | `eks-cluster-v1.1.0` | `/aws/eks/lex-mts-fdev-eks-main/cluster`, `Name` tag `lex-mts-fdev-cwl-eks` |
...
| Bootstrap managed node group and its launch template | `eks-node-group-v1.0.0` | `lex-mts-fdev-ng-bootstrap`, `lex-mts-fdev-lt-bootstrap` |
| Karpenter interruption queue, SQS-managed encryption, 300-second retention | `karpenter-interruption-v1.0.0` | `lex-mts-fdev-sqs-karpenter` |
| The five EventBridge rules that feed it | `karpenter-interruption-v1.0.0` | `lex-mts-fdev-evr-karpsched`, `-karpspot`, `-karprebal`, `-karpstate`, `-karpcapres` |

**What it reads.** It creates no IAM, KMS, or network resource. It reads what
`fdev/networking` and `fdev/security` created, by their standard names:
- the VPC;
...
- **Roles through Pod Identity.** The CNI and the EBS CSI controller take
  fdev/security's roles through EKS Pod Identity, not IRSA. That role exists
...
- **Pinned versions.** The legacy dev cluster's versions are in
  `fdev.tfvars.example`. `eks-pod-identity-agent` is new to this layout. Fill
  its version from:
  ```bash
...
- the cluster security group Amazon EKS creates, for control-plane traffic,
  ...
  it;
- `lex-mts-fdev-sg-node`, for node-to-node traffic and egress.
...
- **The interruption queue** `lex-mts-fdev-sqs-karpenter`. Its policy admits
...
launches take `lex-mts-fdev-role-node`, the bootstrap group's role, which Amazon
...
IRSA role, and it belongs to `fdev/security-irsa`.
...
selectors that find the private subnets fdev/networking tagged and the node
security group fdev/security tagged.
...
- **Public, for named operators only.** Naming operator addresses opens the
  ...
  gitignored `fdev.tfvars`, and `/0` is refused.
...
delete the cluster. The lifecycle's full-development down transition turns it off in
...
```bash
cp fdev.tfvars.example fdev.tfvars                             # fill the account, addresses, and agent version
cp workload.s3.tfbackend.example workload.s3.tfbackend       # fill from shd/state's outputs
terraform -chdir=aws/environments/fdev/workload init -backend-config=workload.s3.tfbackend
terraform -chdir=aws/environments/fdev/workload plan -input=false -var-file=fdev.tfvars -out=fdev-workload.tfplan
```

An apply uses only that saved plan, after the maintainer approves it
(MTS-IAC-107). It runs after `fdev/networking` and `fdev/security`. The state
