# Plan-time tests of the fdev/workload root against a mocked AWS provider (PC-IAC-018).
mock_provider "aws" {
  alias           = "principal"
  ...
  mock_data "aws_iam_role" {
    defaults = { arn = "arn:aws:iam::123456789012:role/lex-mts-fdev-role-cluster" }
  }
  ...
  mock_resource "aws_eks_node_group" {
    defaults = { arn = "arn:aws:eks:us-east-1:123456789012:nodegroup/lex-mts-fdev-eks-main/lex-mts-fdev-ng-bootstrap/00000000-0000-0000-0000-000000000000" }
  }
  ...
  mock_resource "aws_cloudwatch_event_rule" {
    defaults = { arn = "arn:aws:events:us-east-1:123456789012:rule/lex-mts-fdev-evr-karpsched" }
  }
}
...
override_module {
  ...
  outputs = { node_group_arn = "arn:aws:eks:us-east-1:123456789012:nodegroup/lex-mts-fdev-eks-main/lex-mts-fdev-ng-bootstrap/00000000-0000-0000-0000-000000000000" }
}

override_data {
  target = data.aws_iam_role.addon["vpccni"]
  values = { arn = "arn:aws:iam::123456789012:role/lex-mts-fdev-role-vpccni" }
}

override_data {
  target = data.aws_iam_role.addon["ebscsi"]
  values = { arn = "arn:aws:iam::123456789012:role/lex-mts-fdev-role-ebscsi" }
}
...
run "builds_the_fdev_workload_names" {
  command = plan

  assert {
    condition     = local.cluster_name == "lex-mts-fdev-eks-main" && local.node_group_name == "lex-mts-fdev-ng-bootstrap" && local.launch_template_name == "lex-mts-fdev-lt-bootstrap" && local.control_plane_log_group.standard_name == "lex-mts-fdev-cwl-eks"
    error_message = "The root must build the fdev standard names (MTS-IAC-101)."
  }

  assert {
    condition     = local.cluster_security_group_name == "lex-mts-fdev-sg-cluster" && local.node_security_group_name == "lex-mts-fdev-sg-node" && local.addon_role_names == { vpccni = "lex-mts-fdev-role-vpccni", ebscsi = "lex-mts-fdev-role-ebscsi" }
    error_message = "The root must read fdev/security's groups and roles by the names that root gave them."
  }
}
...
run "gives_the_aws_addons_their_pod_identity_roles" {
  ...
  assert {
    condition     = local.addons["vpc-cni"].pod_identity_associations == { aws-node = "arn:aws:iam::123456789012:role/lex-mts-fdev-role-vpccni" } && local.addons["aws-ebs-csi-driver"].pod_identity_associations == { ebs-csi-controller-sa = "arn:aws:iam::123456789012:role/lex-mts-fdev-role-ebscsi" }
...
run "builds_the_karpenter_interruption_names" {
  ...
  assert {
    condition     = local.karpenter_queue.name == "lex-mts-fdev-sqs-karpenter"
    error_message = "The root must build the interruption queue's standard name (MTS-IAC-101)."
  }

  assert {
    condition     = local.karpenter_rule_names == { scheduled_change = "lex-mts-fdev-evr-karpsched", spot_interruption = "lex-mts-fdev-evr-karpspot", rebalance = "lex-mts-fdev-evr-karprebal", instance_state_change = "lex-mts-fdev-evr-karpstate", capacity_reservation = "lex-mts-fdev-evr-karpcapres" }
