# The fdev/workload root, third in the PC-IAC-022 order: the full-development cluster, its managed
# add-ons, and the bootstrap node group. The roles, keys, security groups, and subnets
# come from fdev/security and fdev/networking.
module "eks_cluster" {
...
# and fdev/security's node group, for node-to-node traffic and egress.
...
# fdev/networking and fdev/security put on them.
