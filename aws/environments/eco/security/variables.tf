# Inputs of the eco/security root. Values come from the gitignored eco.tfvars; the account is
...
variable "environment" {
  type        = string
  description = "Environment code, from MTS-IAC-101. This root is the economical environment's security."
...
variable "jwt_environment_codes" {
  type        = list(string)
  description = "Three-letter codes of the application environments the shared cluster hosts, such as dev, stg, prd, and dmo; each gets its JWT signing secret, <client>-<project>-eco-sm-jwt<code>."
