# Inputs of the eco/security-irsa root. Values come from the gitignored eco.tfvars; the account
...
variable "environment" {
  type        = string
  description = "Environment code, from MTS-IAC-101. This root is the economical environment's IRSA pass."
...
variable "jwt_reader_namespaces" {
  type        = map(string)
  description = "Kubernetes namespace of each application environment, keyed by its three-letter code, such as dev = microtodo-dev; its external-secrets-jwt service account reads <client>-<project>-eco-sm-jwt<code>."
