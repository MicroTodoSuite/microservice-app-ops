# Plan-time contract of the fprd/networking transit spoke.
mock_provider "aws" {
  alias = "principal"
...
run "builds_a_three_zone_transit_spoke_without_nat" {
  ...
  assert {
    ...
    error_message = "The full-production spoke needs three public and three private subnets and no NAT gateway of its own."
  }

  assert {
    condition = alltrue([
      for key in ["priva", "privb", "privc"] : local.subnets[key].egress == "transit" && local.subnets[key].route_table_name == "lex-mts-fprd-rtb-priv${substr(key, 4, 1)}"
    ])
    error_message = "Every private fprd subnet must route through its own standard-named transit table."
  }

  assert {
    condition     = local.transit_attachment.name == "lex-mts-fprd-tgwa-spoke" && toset(local.transit_attachment.subnet_keys) == toset(["priva", "privb", "privc"])
    error_message = "The fprd attachment must use all three private subnets and a standard spoke name."
  }
}
...
run "reads_the_shared_hub_and_spoke_tables_by_standard_name" {
  ...
  assert {
    condition     = toset(one([for filter in data.aws_ec2_transit_gateway_route_table.spoke[0].filter : filter if filter.name == "tag:Name"]).values) == toset(["lex-mts-fprd-rtb-tgw"])
...
run "removes_only_the_transit_egress_when_transit_is_disabled" {
  ...
  assert {
    condition     = toset(keys(local.subnets)) == toset(["puba", "pubb", "pubc", "priva", "privb", "privc"]) && alltrue([for key in ["priva", "privb", "privc"] : local.subnets[key].egress == "none" && local.subnets[key].route_table_name == "lex-mts-fprd-rtb-priv${substr(key, 4, 1)}"])
