...
run "reads_the_shared_hub_and_spoke_tables_by_standard_name" {
  ...
  assert {
    condition     = toset(one([for filter in data.aws_ec2_transit_gateway_route_table.spoke.filter : filter if filter.name == "tag:Name"]).values) == toset(["lex-mts-fprd-rtb-tgw"])
