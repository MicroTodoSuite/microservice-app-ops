...
The full-production environment's security root, second in the PC-IAC-022 order
(ops spec 004 T011, ai-agents specs/001 T025). It holds what `fprd/workload`
needs before it can create the cluster.

| Resource | Module | Name |
| --- | --- | --- |
| Control-plane role: `AmazonEKSClusterPolicy`, and use of the secrets key | `iam-role-v1.0.0` | `lex-mts-fprd-role-cluster` |
| Managed nodes' role: `AmazonEKSWorkerNodePolicy`, `AmazonEC2ContainerRegistryPullOnly` | `iam-role-v1.0.0` | `lex-mts-fprd-role-node` |
| Pod Identity role of the `vpc-cni` add-on: `AmazonEKS_CNI_Policy` | `iam-role-v1.0.0` | `lex-mts-fprd-role-vpccni` |
| Pod Identity role of the `aws-ebs-csi-driver` add-on: `AmazonEBSCSIDriverPolicy` | `iam-role-v1.0.0` | `lex-mts-fprd-role-ebscsi` |
| Key for Kubernetes secrets | `kms-key-v1.0.0` | `alias/lex-mts-fprd-kms-eks` |
| Key for the control-plane log group | `kms-key-v1.0.0` | `alias/lex-mts-fprd-kms-ekslogs` |
| Extra control-plane security group | `security-group-v1.0.0` | `lex-mts-fprd-sg-cluster` |
| Node security group | `security-group-v1.0.0` | `lex-mts-fprd-sg-node` |
| JWT signing secrets, one per application environment | `secret-v1.0.0` | `lex-mts-fprd-sm-jwt{dev,stg,prd,dmo}` |
| Slack webhook secrets | `secret-v1.0.0` | `lex-mts-fprd-sm-slackobs`, `lex-mts-fprd-sm-slacksec` |
...
- The logs key lets CloudWatch Logs in this region use it only for
  `/aws/eks/lex-mts-fprd-eks-main/cluster`.
...
**The VPC** is read from `fprd/networking` by its standard name.
...
`lex-mts-fprd-eks-main`, through the session tags Pod Identity sets
(`eks-cluster-arn`, `kubernetes-namespace`, `kubernetes-service-account`).
`fprd/workload` associates the roles through `eks-cluster`'s
`pod_identity_associations`, with the `eks-pod-identity-agent` add-on.

**Not here, because they come in the IRSA pass after `fprd/workload`** (it
...
terraform -chdir=aws/environments/fprd/security init -backend-config=security.s3.tfbackend
terraform -chdir=aws/environments/fprd/security plan -input=false -var-file=fprd.tfvars -out=fprd-security.tfplan
```

An apply uses only that saved plan, after the maintainer approves it
(MTS-IAC-107), and after `fprd/networking`. The state key is new, so the apply
