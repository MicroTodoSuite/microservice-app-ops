# Plan-time tests of the fprd/security root against a mocked AWS provider (PC-IAC-018).
mock_provider "aws" {
  alias           = "principal"
...
run "builds_the_fprd_security_names" {
  command = plan

  assert {
    condition     = local.cluster_role_name == "lex-mts-fprd-role-cluster" && local.node_role_name == "lex-mts-fprd-role-node" && local.secrets_key_name == "lex-mts-fprd-kms-eks" && local.logs_key_name == "lex-mts-fprd-kms-ekslogs"
    error_message = "The root must build the fprd standard names (MTS-IAC-101)."
  }

  assert {
    condition     = local.jwt_secret_names == { dev = "lex-mts-fprd-sm-jwtdev", stg = "lex-mts-fprd-sm-jwtstg", prd = "lex-mts-fprd-sm-jwtprd", dmo = "lex-mts-fprd-sm-jwtdmo" } && local.webhook_secrets["slacksec"].name == "lex-mts-fprd-sm-slacksec"
...
run "trusts_only_each_addon_service_account_through_pod_identity" {
  ...
  assert {
    condition     = local.addon_pod_identities["vpccni"].role_name == "lex-mts-fprd-role-vpccni" && local.addon_pod_identities["ebscsi"].role_name == "lex-mts-fprd-role-ebscsi"
  ...
  assert {
    condition     = jsondecode(local.addon_trust_policies["vpccni"]).Statement[0].Condition.StringEquals == { "aws:RequestTag/eks-cluster-arn" = "arn:aws:eks:us-east-1:123456789012:cluster/lex-mts-fprd-eks-main", "aws:RequestTag/kubernetes-namespace" = "kube-system", "aws:RequestTag/kubernetes-service-account" = "aws-node" }
...
run "limits_the_logs_key_to_the_control_plane_log_group" {
  ...
  assert {
    condition     = jsondecode(local.logs_key_policy).Statement[1].Principal.Service == "logs.us-east-1.amazonaws.com" && jsondecode(local.logs_key_policy).Statement[1].Condition.ArnEquals["kms:EncryptionContext:aws:logs:arn"] == "arn:aws:logs:us-east-1:123456789012:log-group:/aws/eks/lex-mts-fprd-eks-main/cluster"
