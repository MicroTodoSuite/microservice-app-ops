# Inputs of the fprd/security root. Values come from the gitignored fprd.tfvars; the account is
...
variable "environment" {
  type        = string
  description = "Environment code, from MTS-IAC-101. This root is the full-production environment's security."
...
variable "jwt_environment_codes" {
  type        = list(string)
  description = "Three-letter codes of the application environments the shared cluster hosts, such as dev, stg, prd, and dmo; each gets its JWT signing secret, <client>-<project>-fprd-sm-jwt<code>."
