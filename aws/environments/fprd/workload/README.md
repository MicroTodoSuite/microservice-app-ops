...
The full-production environment's workload, third in the PC-IAC-022 order (ops
spec 004 T011, ai-agents specs/001 T025). It creates the cluster and its
bootstrap capacity.

| Resource | Module | Name |
| --- | --- | --- |
| EKS cluster, Kubernetes `kubernetes_version`, private API plus an optional public endpoint | `eks-cluster-v1.1.0` | `lex-mts-fprd-eks-main` |
| Control-plane log group, encrypted with fprd/security's logs key | `eks-cluster-v1.1.0` | `/aws/eks/lex-mts-fprd-eks-main/cluster`, `Name` tag `lex-mts-fprd-cwl-eks` |
...
- **Roles through Pod Identity.** The CNI and the EBS CSI controller take
  fprd/security's roles through EKS Pod Identity, not IRSA. That role exists
...
- **Pinned versions.** The legacy dev cluster's versions are in
  `fprd.tfvars.example`. `eks-pod-identity-agent` is new to this layout. Fill
  its version from:
  ```bash
...
- the cluster security group Amazon EKS creates, for control-plane traffic,
  ...
  it;
- `lex-mts-fprd-sg-node`, for node-to-node traffic and egress.
...
delete the cluster. The lifecycle's full-production down transition turns it off in
...
```bash
cp fprd.tfvars.example fprd.tfvars                             # fill the account, addresses, and agent version
cp workload.s3.tfbackend.example workload.s3.tfbackend       # fill from shd/state's outputs
terraform -chdir=aws/environments/fprd/workload init -backend-config=workload.s3.tfbackend
terraform -chdir=aws/environments/fprd/workload plan -input=false -var-file=fprd.tfvars -out=fprd-workload.tfplan
```

An apply uses only that saved plan, after the maintainer approves it
(MTS-IAC-107). It runs after `fprd/networking` and `fprd/security`. The state
