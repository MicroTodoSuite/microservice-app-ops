# Inputs of the fprd/workload root. Values come from the gitignored fprd.tfvars; the account is
...
variable "environment" {
  type        = string
  description = "Environment code, from MTS-IAC-101. This root is the full-production environment's workload."
...
variable "endpoint_public_access_cidrs" {
  type        = list(string)
  description = "Operator CIDR blocks allowed to reach the public API endpoint, such as /32 addresses; [] keeps the API private. Real addresses belong only in the gitignored fprd.tfvars."
