# Contract for the full-profile dev environment.
#
...
mock_provider "aws" {
  ...
  mock_data "aws_secretsmanager_secret" {
    defaults = {
      arn = "arn:aws:secretsmanager:us-east-1:916491575487:secret:microtodosuite/dev/auth-api-secrets-AbCdEf"
      id  = "arn:aws:secretsmanager:us-east-1:916491575487:secret:microtodosuite/dev/auth-api-secrets-AbCdEf"
    }
  }
}
...
override_module {
  ...
  outputs = {
    ...
    public_subnets  = ["subnet-fd-public-a", "subnet-fd-public-b", "subnet-fd-public-c"]
    private_subnets = ["subnet-fd-private-a", "subnet-fd-private-b", "subnet-fd-private-c"]

    private_route_table_ids = ["rtb-fd-private-a", "rtb-fd-private-b", "rtb-fd-private-c"]
    public_route_table_ids  = ["rtb-fd-public"]
    ...
    vpc_flow_log_id              = "fl-0fulldev000000000"
    vpc_flow_log_destination_arn = "arn:aws:logs:us-east-1:916491575487:log-group:/aws/vpc-flow-logs/microtodosuite-full-dev"
  }
}
...
override_module {
  ...
  outputs = {
    ...
    cluster_oidc_issuer_url            = "https://oidc.eks.us-east-1.amazonaws.com/id/fulldev"
    oidc_provider                      = "oidc.eks.us-east-1.amazonaws.com/id/fulldev"
    oidc_provider_arn                  = "arn:aws:iam::916491575487:oidc-provider/oidc.eks.us-east-1.amazonaws.com/id/fulldev"
  }
}
...
override_module {
  ...
  outputs = {
    iam_role_arn          = "arn:aws:iam::916491575487:role/microtodosuite-full-dev-karpenter"
    node_iam_role_arn     = "arn:aws:iam::916491575487:role/microtodosuite-full-dev-karpenter-node"
    node_iam_role_name    = "microtodosuite-full-dev-karpenter-node"
    instance_profile_name = "microtodosuite-full-dev-karpenter-node"
    queue_arn             = "arn:aws:sqs:us-east-1:916491575487:microtodosuite-full-dev-karpenter"
    queue_name            = "microtodosuite-full-dev-karpenter"
    queue_url             = "https://sqs.us-east-1.amazonaws.com/916491575487/microtodosuite-full-dev-karpenter"
...
override_resource {
  ...
  values = {
    arn = "arn:aws:iam::916491575487:role/microtodosuite-full-dev-aws-load-balancer-controller"
  }
}
...
override_resource {
  ...
  values = {
    arn = "arn:aws:iam::916491575487:role/microtodosuite-full-dev-dev-jwt-reader"
  }
}
...
run "identity_region_and_address_space_match_the_reviewed_allocation" {
  ...
  assert {
    ...
    error_message = "full-dev belongs to the single reviewed AWS account."
  }

  assert {
    condition     = var.aws_region == "us-east-1"
    error_message = "full-dev must be in us-east-1, the region of the shared egress hub it depends on."
  }

  assert {
    condition     = var.vpc_cidr == "10.40.0.0/16"
    error_message = "full-dev owns 10.40.0.0/16. Any other range risks colliding with a sibling environment on the shared transit gateway."
  }

  assert {
    condition     = output.foundation_contract.environment == "full-dev"
    error_message = "The root must build the full-dev environment and nothing else."
  }
}
...
run "private_workloads_leave_through_the_shared_hub_not_a_local_nat" {
  ...
  assert {
    ...
    error_message = "full-dev must use the shared egress hub."
  }
  ...
  assert {
    ...
    error_message = "full-dev must consume zero NAT gateways and therefore zero Elastic IPs; the EIP quota is the binding constraint across the fleet."
  }

  assert {
    condition     = output.foundation_contract.network.transit_gateway_id == "tgw-0123456789abcdef0"
    error_message = "full-dev must point at the reviewed shared transit gateway."
  }
...
run "bootstrap_capacity_is_a_single_node" {
  ...
  assert {
    ...
    error_message = "full-dev bootstraps on exactly one node; Karpenter provisions everything after that."
  }
...
run "full_profile_cluster_prerequisites_are_enabled" {
  ...
  assert {
    ...
    error_message = "full-dev is a full-profile environment and must enable its cluster prerequisites."
  }
...
run "consumer_mode_creates_no_shared_resource_and_no_secret" {
  ...
  assert {
    ...
    error_message = "full-dev is a consumer environment; the dev owner root owns every shared resource."
  }

  assert {
    condition     = output.foundation_contract.shared_resources.owned == false
    error_message = "full-dev must report itself as a consumer, not an owner, of the shared resources."
  }
...
run "exactly_one_cluster_qualified_dev_jwt_reader" {
  command = plan
  ...
  assert {
    ...
    error_message = "full-dev needs exactly one reader for the dev JWT secret."
  }

  assert {
    condition     = output.foundation_contract.consumer_jwt.environment == "dev"
    error_message = "full-dev reads the dev JWT secret; reading staging or prod would cross an environment boundary."
  }
