variable "environment" {
  ...
  validation {
    condition     = var.environment == "full-dev"
    error_message = "This root builds full-dev only. Another environment name here would create a second cluster from full-dev's state key."
  }
}

variable "runtime_enabled" {
  description = "Whether the full-development VPC, EKS cluster, nodes, add-ons, and cluster-scoped identities are present."
  type        = bool
  default     = true
}

variable "expected_account_id" {
  description = "AWS account allowed to receive the full-dev foundation."
  type        = string
...
variable "aws_region" {
  description = "Region for full-dev. A transit gateway attachment is regional, so this must match the shared egress hub."
  type        = string
  default     = "us-east-1"

  validation {
    condition     = var.aws_region == "us-east-1"
    error_message = "full-dev must be in us-east-1, where the shared egress hub it attaches to lives."
  }
}
...
variable "vpc_cidr" {
  description = "Address space reserved for full-dev. Must not overlap any sibling spoke on the shared transit gateway."
  type        = string
  default     = "10.40.0.0/16"

  validation {
    condition     = var.vpc_cidr == "10.40.0.0/16"
    error_message = "full-dev owns 10.40.0.0/16 in the reviewed allocation. Overlapping a sibling spoke silently breaks transit routing rather than failing at apply."
  }
}

variable "availability_zones" {
  description = "Availability zones for full-dev."
  type        = list(string)
...
variable "outbound_mode" {
  description = "How private workloads reach the internet. full-dev uses the shared egress hub and creates no NAT gateway of its own."
  type        = string
  default     = "transit-egress"

  validation {
    condition     = var.outbound_mode == "transit-egress"
    error_message = "full-dev is a spoke of the shared egress hub. Switching it to direct-nat would add a NAT gateway and an Elastic IP that the fleet quota does not budget for."
  }
}
...
variable "transit_gateway_id" {
  ...
  validation {
    ...
    error_message = "full-dev uses transit egress and therefore requires a transit gateway id. Without one its private subnets would have no default route at all and every worker would come up with no path off-VPC."
  }
}
...
variable "create_shared_resources" {
  description = "full-dev is a consumer. Every shared resource is owned by the dev owner root; creating them here would produce a second copy of an account-level singleton."
  type        = bool
  default     = false

  validation {
    condition     = var.create_shared_resources == false
    error_message = "full-dev must stay a consumer environment. Shared ECR, the GitHub OIDC provider, and the JWT secret containers are owned by aws/environments/dev/foundation."
  }
}
...
variable "consumer_jwt_environment" {
  ...
  validation {
    condition     = var.consumer_jwt_environment == "dev"
    error_message = "full-dev reads the dev JWT secret only. Reading staging or prod would cross an environment boundary from a development cluster."
  }
}
...
variable "enable_full_profile_cluster_prerequisites" {
  description = "full-dev is a full-profile environment: EBS CSI, Karpenter, and the AWS Load Balancer Controller roles are all required."
  type        = bool
  default     = true
