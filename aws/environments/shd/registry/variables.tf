# Inputs of the shd/registry root. Values come from the gitignored shd.tfvars; the account is
...
variable "environment" {
  type        = string
  description = "Environment code, from MTS-IAC-101. The service image registry is shared, so always shd."

  validation {
    condition     = var.environment == "shd"
    error_message = "The service image registry belongs to the shared environment, shd."
  }
}
...
variable "service_image_keys" {
  type        = list(string)
  description = "Keys of the service image repositories, such as authapi; each becomes <client>-<project>-shd-ecr-<key>, the names shd/security's publisher role may push to."
