...
resource "aws_secretsmanager_secret_version" "groq_api_key" {
  ...
  secret_string_wo         = var.groq_api_key_wo
  secret_string_wo_version = var.groq_api_key_wo_version
}
...
resource "aws_lambda_function" "github_webhook_handler" {
  ...
  environment {
    variables = {
      ...
      GROQ_API_KEY_SECRET_ARN    = aws_secretsmanager_secret.groq_api_key.arn
